...
const ChatUi = ({ member, userId, name, socket, setMsgCounts,setSelectedFriend }) => {
  ...
  const sendMessage = async () => {
    if (messageInput.trim() !== '') {
      ...
      await fetch(`https://api.makethechange.in/sendMessage`, {
        method: 'POST',
        headers: {
  ...
  useEffect(() => {
    const handleSetStatus = async (data) => {
      ...
      await fetch(`https://api.makethechange.in/updateMessageStatus`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const clearChat = async () => {
    try {
      const response = await fetch(`https://api.makethechange.in/deleteChat`, {
          method: 'POST',
          headers: {
              'Content-Type': 'application/json',
          },
