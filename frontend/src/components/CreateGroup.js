...
const CreateGroupDrawer = ({ friends, isOpen, onClose, onGroupCreated }) => {
  ...
  const handleCreateGroup = async () => {
    ...
    if (missingKeys.length > 0 || !user?.publicKey) {
      return setError('Some members are missing encryption keys and can\'t be added to a group yet');
    }
    ...
    try {
      // One symmetric key for the whole group, wrapped once per member
      // (including yourself, so you can still read your own messages).
      const groupKey = await CryptoUtils.generateGroupKey();
      const encryptedKeys = await CryptoUtils.wrapGroupKeyForMembers(groupKey, [
        { userId: user._id, publicKey: user.publicKey },
        ...selectedUsers.map((u) => ({ userId: u.value, publicKey: u.publicKey })),
      ]);
